...
const PROPERTY_TYPE_ITEMS: { value: string; translationKey: "apartment" | "house" | "studio" | "loft" | "room"; Icon: IconComponent }[] = [
  { value: "apartment", translationKey: "apartment", Icon: MapPin },
  { value: "house", translationKey: "house", Icon: MapPin },
  { value: "studio", translationKey: "studio", Icon: Sofa },
  { value: "loft", translationKey: "loft", Icon: Maximize },
  { value: "room", translationKey: "room", Icon: Bed },
];
...
const OPTIONS: { key: keyof Filters; translationKey: "furnished" | "petsAllowed" | "utilitiesIncluded"; Icon: IconComponent }[] = [
  { key: "furnished", translationKey: "furnished", Icon: Sofa },
  { key: "pets_allowed", translationKey: "petsAllowed", Icon: Dog },
  { key: "utilities_included", translationKey: "utilitiesIncluded", Icon: FileText },
];
  ...
  return (
    <div className="absolute inset-0 z-[9999] bg-white flex flex-col animate-filter-open">
      <div className="flex items-center justify-between px-5 pt-5 pb-4 shrink-0">
        <button type="button" onClick={onClose} aria-label={tCommon("close")} className="w-9 h-9 rounded-full bg-gray-100 flex items-center justify-center text-gray-600 hover:bg-gray-200 transition-colors">
          <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" className="w-4 h-4">
            <path strokeLinecap="round" strokeLinejoin="round" d="M15 19l-7-7 7-7" />
          </svg>
        </button>
        <span className="text-title-sm font-bold text-gray-900">{t("filters")}</span>
      ...
      <div className="flex-1 overflow-y-auto scrollbar-hide">
        <div className="px-5 py-2 space-y-7">
          ...
          <section>
            <p className="text-body-sm font-bold text-gray-400 uppercase tracking-widest mb-4">{t("propertyType")}</p>
            <div className="grid grid-cols-3 gap-2">
              {PROPERTY_TYPE_ITEMS.map(({ value, translationKey, Icon }) => {
                const active = filters.property_type === value;
                return (
                  <button
                    key={value}
                    type="button"
                    onClick={() => onChange({ ...filters, property_type: active ? null : value })}
                  ...
                  >
                    ...
                    <span className={`text-body-xs font-semibold ${active ? "text-gray-900" : "text-gray-500"}`}>{t(translationKey)}</span>
                  </button>
                );
              })}
            </div>
          ...
          <section>
            <p className="text-body-sm font-bold text-gray-400 uppercase tracking-widest mb-4">{t("maxRent")}</p>
            <Pills
              options={RENTS}
              value={filters.max_rent as typeof RENTS[number] | null}
              onChange={(v) => onChange({ ...filters, max_rent: v })}
              format={(v, i) => i === RENTS.length - 1 ? `${v}${tCommon("currency")}+` : `${v}${tCommon("currency")}`}
            />
          </section>
          ...
          <section>
            <p className="text-body-sm font-bold text-gray-400 uppercase tracking-widest mb-4">{t("minSurface")}</p>
            <Pills
              options={SURFACES}
              value={filters.min_surface as typeof SURFACES[number] | null}
              onChange={(v) => onChange({ ...filters, min_surface: v })}
              format={(v, i) => i === SURFACES.length - 1 ? `${v}${tCommon("sqm")}+` : `${v}${tCommon("sqm")}`}
            />
          </section>
          ...
          <section>
            <p className="text-body-sm font-bold text-gray-400 uppercase tracking-widest mb-4">{t("minRooms")}</p>
            <Pills
              options={ROOMS}
          ...
          <section>
            <p className="text-body-sm font-bold text-gray-400 uppercase tracking-widest mb-4">{t("options")}</p>
            <div className="space-y-4">
              {OPTIONS.map(({ key, translationKey, Icon }) => (
                <div key={key} className="flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    <Icon size={20} className="text-gray-500" />
                    <span className="text-body-md font-semibold text-gray-800">{t(translationKey)}</span>
                  </div>
