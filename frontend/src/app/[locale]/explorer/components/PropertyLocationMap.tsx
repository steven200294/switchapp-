...
export default function PropertyLocationMap({ latitude, longitude, city }: PropertyLocationMapProps) {
  ...
  return (
    <div className="mb-8 relative z-0">
      <h3 className="text-title font-bold text-gray-900 mb-4">{t("location")}</h3>
      <div className="rounded-2xl overflow-hidden border border-gray-100 shadow-sm relative z-0">
        ...
        <div className="px-4 py-3 bg-white flex items-center gap-2">
          ...
          <p className="text-body-sm text-gray-500 font-medium">
            {t.rich("addressAfterSwitch", {
              city,
              highlight: (chunks) => <span className="font-bold text-gray-900">{chunks}</span>,
            })}
          </p>
        </div>
      </div>
