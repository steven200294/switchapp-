...
export default function ProfilPublicPage() {
  ...
  const name = user?.full_name ?? tCommon("user");
    ...
    <div className="fixed inset-0 z-[100] flex flex-col justify-end md:justify-center md:items-center">
      ...
      <div className="relative w-full md:max-w-lg h-[92vh] md:h-auto md:max-h-[85vh] bg-white rounded-t-3xl md:rounded-3xl shadow-2xl flex flex-col animate-page-slide-up overflow-hidden">
        ...
        <header className="px-5 py-3 border-b border-gray-100 flex items-center justify-between sticky top-0 bg-white/95 backdrop-blur-sm z-10 shrink-0">
          <div className="w-10" />
          <h1 className="text-body-xl font-bold text-gray-900">{t("publicProfile")}</h1>
          <Link href="/profil" scroll={false} aria-label={t("closeModal")} className="w-10 h-10 rounded-full hover:bg-gray-100 transition-colors flex items-center justify-center text-gray-400 hover:text-gray-900">
        ...
        <main className="flex-1 overflow-y-auto">
          ...
          <div className="pt-14 px-6 pb-8">
            ...
            <div className="text-center mb-6">
              ...
              <p className="text-body-sm text-gray-400 mt-1">{t("memberSinceYear", { year: joined })}</p>
            </div>

            {/* Stats */}
            <div className="grid grid-cols-3 gap-3 mb-8">
              {[
                { id: "switches", label: t("switches"), value: "0" },
                { id: "reviews", label: t("reviews"), value: "0" },
                { id: "rating", label: t("rating"), value: "—" },
              ].map(({ id, label, value }) => (
                <div key={id} className="bg-gray-50 rounded-2xl p-4 text-center">
            ...
            {/* À propos */}
            <div className="mb-8">
              <h3 className="text-body-sm font-bold text-gray-400 uppercase tracking-widest mb-3">{t("about")}</h3>
            ...
            {/* Avis */}
            <div>
              <h3 className="text-body-sm font-bold text-gray-400 uppercase tracking-widest mb-3">{t("reviews")}</h3>
              <div className="flex flex-col items-center py-8 gap-3 text-center">
                <Star size={32} className="text-gray-200" aria-hidden />
                <p className="text-body-sm text-gray-400 font-medium">{t("noReviews")}</p>
                <p className="text-body-xs text-gray-300">{t("reviewsWillAppear")}</p>
              </div>
            </div>
          </div>
        </main>
