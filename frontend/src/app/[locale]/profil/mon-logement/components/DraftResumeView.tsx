...
import { seedDraftFromProperty } from "../services/draft-seed";
import { usePublishById, useDeleteProperty, useUpdateProperty } from "../hooks/usePropertyMutations";
...
export default function DraftResumeView({ property: p, onClose }: Props) {
  const t = useTranslations("propertyManage");
  const [mode, setMode] = useState<"overview" | "chatbot">("overview");
  const [showDeleteConfirm, setShowDeleteConfirm] = useState(false);

  const publishMutation = usePublishById(onClose);
  const deleteMutation = useDeleteProperty(onClose);
  const updateMutation = useUpdateProperty();
  const actionLoading = publishMutation.isPending || deleteMutation.isPending;
  ...
  if (mode === "chatbot") {
    return <PropertyChatBot onClose={onClose} />;
  }
    ...
    <div className="flex flex-col h-full">
      <header className="flex items-center gap-3 px-4 py-4 border-b border-gray-100 bg-white sticky top-0 z-10">
        <div className="w-full max-w-2xl mx-auto flex items-center gap-3">
          ...
          <span className="px-3 py-1 rounded-full bg-amber-100 text-amber-700 text-body-sm font-semibold">
            {t("draft")}
          </span>
        </div>
      </header>
      ...
      <div className="flex-1 overflow-y-auto pb-44">
        <div className="max-w-2xl mx-auto px-4">
          <div className="mt-4 bg-amber-50 border border-amber-200 rounded-2xl px-4 py-3 text-body-sm text-amber-700 font-medium">
            {t("draftBanner")}
          </div>
          ...
          <div className="mt-5 bg-white rounded-2xl border border-gray-100 p-4">
            ...
            <EditableNumberField label={t("rent")} value={p.monthly_rent} onSave={(v) => save({ monthly_rent: v })} saving={updateMutation.isPending} suffix="€" min={0} />
            <EditableNumberField label={t("deposit")} value={p.deposit} onSave={(v) => save({ deposit: v })} saving={updateMutation.isPending} suffix="€" min={0} />
          </div>
        ...
        <div className="max-w-2xl mx-auto space-y-3">
          <div className="flex gap-3">
            <button
              ...
              className="flex-1 py-3.5 rounded-2xl bg-brand-dark text-white font-bold text-body-md active:scale-[0.97] transition-all outline-none"
            >
              {t("continueDraft")}
            </button>
            {canPublish && (
              <button
                ...
                className="flex-1 py-3.5 rounded-2xl bg-linear-to-r from-brand-cyan to-brand-purple text-white font-bold text-body-md active:scale-[0.97] transition-all outline-none"
              >
          ...
          <button
            ...
            className="w-full flex items-center justify-center gap-2 py-2.5 text-red-500 font-medium text-body-sm outline-none"
          >
      ...
      {showDeleteConfirm && (
        <div className="fixed inset-0 z-200 bg-black/40 flex items-end md:items-center justify-center" onClick={() => setShowDeleteConfirm(false)}>
          <div className="bg-white rounded-t-3xl md:rounded-3xl w-full max-w-md px-6 py-6 space-y-4 animate-chat-slide-up" onClick={(e) => e.stopPropagation()}>
            <p className="text-body-lg font-bold text-gray-900 text-center">{t("delete")}</p>
            <p className="text-body text-gray-500 text-center">{t("confirmDelete")}</p>
            <button
              type="button"
              onClick={() => deleteMutation.mutate(p.id)}
              disabled={actionLoading}
              className="w-full py-3.5 rounded-2xl bg-red-500 text-white font-bold text-body-md outline-none"
            >
            ...
            <button type="button" onClick={() => setShowDeleteConfirm(false)} className="w-full py-3 text-gray-500 font-semibold text-body-md outline-none">
              {t("cancel")}
            </button>
          </div>
        </div>
