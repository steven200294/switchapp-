...
export default function SwipeOverlays({ likeOpacity, nopeOpacity }: SwipeOverlaysProps) {
  ...
  return (
    <>
      ...
      >
        <span className="text-brand-cyan text-display-xs font-black">{t("switchLabel")}</span>
      </motion.div>
      <motion.div
        className="absolute top-8 right-8 z-10 border-4 border-brand-purple rounded-2xl px-6 py-2 rotate-[20deg]"
        style={{ opacity: nopeOpacity }}
      >
        <span className="text-brand-purple text-display-xs font-black">{t("nopeLabel")}</span>
      </motion.div>
    </>
  );
}
