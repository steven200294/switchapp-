...
export default function SearchModal({ properties, onClose, onSelectProperty }: SearchModalProps) {
      ...
      <div className="search-modal-panel relative z-10 bg-white w-full h-full md:h-full md:w-[75vw] md:max-w-5xl md:shadow-2xl flex flex-col overflow-hidden">
        <div className="px-4 pt-4 pb-3 border-b border-gray-100 shrink-0">
          <div className="flex items-center gap-3">
            <button type="button" onClick={onClose} className="w-9 h-9 rounded-full bg-gray-100 flex items-center justify-center text-gray-600 hover:bg-gray-200 transition-colors shrink-0">
        ...
        <div className="flex-1 flex flex-col md:flex-row overflow-hidden">
                ...
                <div className="flex items-center gap-3 border border-gray-200 rounded-2xl p-3 bg-white shadow-lg">
                  <div className="w-16 h-16 rounded-xl bg-gray-100 overflow-hidden shrink-0">
                    {selectedProperty.cover_image && (
                      <img src={selectedProperty.cover_image} alt="" className="w-full h-full object-cover" />
                    )}
                  </div>
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-body-sm font-bold text-gray-900 mt-0.5">{selectedProperty.monthly_rent}€<span className="font-normal text-gray-400"> /mois</span></p>
                  </div>
                  <button
                    type="button"
          ...
          <div className="flex-1 md:w-[320px] md:flex-none overflow-y-auto scrollbar-hide border-t md:border-t-0 md:border-l border-gray-100">
            <div className="p-4">
              <p className="text-body-sm text-gray-400 font-medium mb-3">{filtered.length} logement{filtered.length !== 1 ? "s" : ""}</p>
              <div className="space-y-3">
                {filtered.map((p) => (
                  <button key={p.id} type="button" onClick={() => handleNavigate(p.id)} className="w-full flex items-center gap-3 p-3 rounded-xl hover:bg-gray-50 transition-colors text-left">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-body-sm font-bold text-gray-900 mt-0.5">{p.monthly_rent}€<span className="font-normal text-gray-400"> /mois</span></p>
                    </div>
                  </button>
                ))}
              </div>
