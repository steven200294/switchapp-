...
export default function StepLocation({ draft, update, onNext }: Props) {
  ...
  return (
    <div className="max-w-xl mx-auto px-6 py-10">
      <h2 className="text-display-sm font-black text-gray-900 mb-2">Où se situe votre logement ?</h2>
      <p className="text-body-md text-gray-400 mb-8">Votre adresse exacte ne sera partagée qu'après confirmation d'un Switch.</p>

      <div className="space-y-5 mb-10">
        <Field label="Adresse">
          <input type="text" value={draft.address} onChange={(e) => update({ address: e.target.value })} placeholder="12 rue de la Paix" className={inputCls} />
        </Field>

        <div className="grid grid-cols-2 gap-4">
          <Field label="Code postal">
            <input type="text" value={draft.postal_code} onChange={(e) => update({ postal_code: e.target.value })} placeholder="75001" className={inputCls} />
          </Field>
          <Field label="Ville">
            <input type="text" value={draft.city} onChange={(e) => update({ city: e.target.value })} placeholder="Paris" className={inputCls} />
          </Field>
        </div>

        <Field label="Quartier (optionnel)">
          <input type="text" value={draft.district} onChange={(e) => update({ district: e.target.value })} placeholder="Marais, Montmartre..." className={inputCls} />
        </Field>
      </div>
