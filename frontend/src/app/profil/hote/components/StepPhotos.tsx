...
export default function StepPhotos({ draft, update, onNext }: Props) {
  ...
  return (
    <div className="max-w-xl mx-auto px-6 py-10">
      <h2 className="text-display-sm font-black text-gray-900 mb-2">Photos de votre logement</h2>
      <p className="text-body-md text-gray-400 mb-8">Ajoutez jusqu'à 10 photos. La première sera votre photo de couverture.</p>
      ...
      >
        ...
        <p className="text-body-md font-bold text-gray-700">Cliquez ou glissez vos photos</p>
        <p className="text-body-sm text-gray-400">JPG, PNG · 10 Mo max par photo</p>
        ...
        <div className="grid grid-cols-3 gap-3 mb-8">
          {previews.map((src, i) => (
            <div key={i} className={`relative aspect-square rounded-xl overflow-hidden ${i === 0 ? "ring-2 ring-black" : ""}`}>
              ...
              {i === 0 && <span className="absolute bottom-1 left-1 bg-black text-white text-body-2xs font-bold px-2 py-0.5 rounded-full">Couverture</span>}
              <button type="button" onClick={() => remove(i)} className="absolute top-1 right-1 w-6 h-6 bg-black/60 rounded-full flex items-center justify-center text-white hover:bg-black transition-colors">
