...
const STEPS = [
  "Type", "Localisation", "Caractéristiques", "Équipements",
  "Photos", "Tarif", "Description", "Récapitulatif",
];
    ...
    <div className="fixed inset-0 z-[100] overflow-hidden">
      ...
      <div className="absolute inset-0 w-full h-full bg-white shadow-2xl flex flex-col animate-page-slide-right overflow-hidden">
        ...
        <header className="px-5 py-4 border-b border-gray-100 flex items-center gap-4 sticky top-0 bg-white/95 backdrop-blur-sm z-10 shrink-0">
          {step > 0 ? (
            <button type="button" onClick={prev} className="w-10 h-10 rounded-full hover:bg-gray-100 transition-colors flex items-center justify-center shrink-0">
            ...
            <Link href="/profil" scroll={false} className="w-10 h-10 rounded-full hover:bg-gray-100 transition-colors flex items-center justify-center shrink-0">
          ...
          <div className="flex-1">
            <p className="text-body-xs text-gray-400 font-medium mb-1">Étape {step + 1} sur {STEPS.length} · {STEPS[step]}</p>
