...
import { getAdminProperties, type AdminProperty } from '@/app/admin/services/admin.service';
...
function PropertyCover({ prop }: { prop: AdminProperty }) {
  ...
  if (src) {
    return <img src={src} alt="" className="w-full h-36 object-cover" />;
  }
  return (
    <div className="w-full h-36 bg-linear-to-br from-gray-100 to-gray-200 flex items-center justify-center text-gray-300 text-4xl">
      🏠
    </div>
  );
}
      ...
      {isLoading ? (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {properties.map((prop) => (
            <div
              key={prop.id}
              onClick={() => router.push(`/admin/users/${prop.owner_id}`)}
            ...
            >
              ...
              <div className="p-4">
                ...
                <div className="flex items-center justify-between text-xs text-gray-400 mt-2">
                  <span>
                    {[prop.rooms && `${prop.rooms} pièces`, prop.surface_area && `${prop.surface_area}m²`]
                      .filter(Boolean).join(' · ')}
                  </span>
                  {prop.monthly_rent ? (
                    <span className="font-semibold text-gray-700">{prop.monthly_rent} €/m</span>
                  ) : null}
                </div>
                <div className="mt-3 pt-3 border-t border-gray-100 flex items-center gap-2">
                  ...
                  {prop.photos?.length > 0 && (
                    <span className="ml-auto text-xs text-gray-400">{prop.photos.length} photos</span>
                  )}
                </div>
              </div>
            </div>
